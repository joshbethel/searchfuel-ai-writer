...
export default function AiVisibilityAdminControls() {
  ...
  const loadAiVisibilityPolicy = async () => {
    ...
    try {
      ...
      const normalizedValue = Math.max(MIN_RUN_COST_USD, Number(data?.max_cost_usd ?? DEFAULT_MAX_COST_USD));
  ...
  return (
    <div className="container mx-auto py-8 px-4 space-y-6">
      <div className="space-y-3">
        ...
        <h1 className="text-3xl font-bold">AI Visibility Controls</h1>
        <p className="text-muted-foreground max-w-2xl">
          Global budget cap and provider policies applied across all sites.
        </p>
      </div>
