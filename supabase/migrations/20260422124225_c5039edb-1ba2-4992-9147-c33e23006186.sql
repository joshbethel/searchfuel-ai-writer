...
-- ai_visibility_settings policies
DROP POLICY IF EXISTS "Users can view own ai visibility settings" ON public.ai_visibility_settings;
CREATE POLICY "Users can view own ai visibility settings" ON public.ai_visibility_settings FOR SELECT
...
CREATE POLICY "Users can insert own ai visibility settings" ON public.ai_visibility_settings FOR INSERT
...
CREATE POLICY "Users can update own ai visibility settings" ON public.ai_visibility_settings FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_settings.blog_id AND b.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_settings.blog_id AND b.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can delete own ai visibility settings" ON public.ai_visibility_settings;
CREATE POLICY "Users can delete own ai visibility settings" ON public.ai_visibility_settings FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_settings.blog_id AND b.user_id = auth.uid()));

-- ai_visibility_prompts policies
DROP POLICY IF EXISTS "Users can view own ai visibility prompts" ON public.ai_visibility_prompts;
CREATE POLICY "Users can view own ai visibility prompts" ON public.ai_visibility_prompts FOR SELECT
...
CREATE POLICY "Users can insert own ai visibility prompts" ON public.ai_visibility_prompts FOR INSERT
...
CREATE POLICY "Users can update own ai visibility prompts" ON public.ai_visibility_prompts FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_prompts.blog_id AND b.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_prompts.blog_id AND b.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can delete own ai visibility prompts" ON public.ai_visibility_prompts;
CREATE POLICY "Users can delete own ai visibility prompts" ON public.ai_visibility_prompts FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_prompts.blog_id AND b.user_id = auth.uid()));

-- ai_visibility_runs policies
DROP POLICY IF EXISTS "Users can view own ai visibility runs" ON public.ai_visibility_runs;
CREATE POLICY "Users can view own ai visibility runs" ON public.ai_visibility_runs FOR SELECT
...
CREATE POLICY "Users can insert own ai visibility runs" ON public.ai_visibility_runs FOR INSERT
...
CREATE POLICY "Users can update own ai visibility runs" ON public.ai_visibility_runs FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_runs.blog_id AND b.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_runs.blog_id AND b.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can delete own ai visibility runs" ON public.ai_visibility_runs;
CREATE POLICY "Users can delete own ai visibility runs" ON public.ai_visibility_runs FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_runs.blog_id AND b.user_id = auth.uid()));

-- ai_visibility_results_raw policies
DROP POLICY IF EXISTS "Users can view own ai visibility raw" ON public.ai_visibility_results_raw;
CREATE POLICY "Users can view own ai visibility raw" ON public.ai_visibility_results_raw FOR SELECT
...
CREATE POLICY "Users can insert own ai visibility raw" ON public.ai_visibility_results_raw FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_results_raw.blog_id AND b.user_id = auth.uid()));

-- ai_visibility_mentions policies
DROP POLICY IF EXISTS "Users can view own ai visibility mentions" ON public.ai_visibility_mentions;
CREATE POLICY "Users can view own ai visibility mentions" ON public.ai_visibility_mentions FOR SELECT
...
CREATE POLICY "Users can insert own ai visibility mentions" ON public.ai_visibility_mentions FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_mentions.blog_id AND b.user_id = auth.uid()));

-- ai_visibility_model_metrics policies
DROP POLICY IF EXISTS "Users can view own ai visibility metrics" ON public.ai_visibility_model_metrics;
CREATE POLICY "Users can view own ai visibility metrics" ON public.ai_visibility_model_metrics FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.blogs b WHERE b.id = ai_visibility_model_metrics.blog_id AND b.user_id = auth.uid()));
DROP POLICY IF EXISTS "Users can insert own ai visibility metrics" ON public.ai_visibility_model_metrics;
CREATE POLICY "Users can insert own ai visibility metrics" ON public.ai_visibility_model_metrics FOR INSERT
